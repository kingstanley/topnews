...
class TopNewsOverViewScreenSmall extends StatefulWidget {
  static final routeName = 'top-news-overview';
  static final apiKey = "9qapiOnguKdzg0jWODFBUMF1YLTAkHYV";

  @override
  _TopNewsOverViewScreenSmallState createState() =>
      _TopNewsOverViewScreenSmallState();
}

class _TopNewsOverViewScreenSmallState
    extends State<TopNewsOverViewScreenSmall> {
  List posts = [];
  ...
  void _getPostsFromVanguard() async {
    try {
      ...
      debugPrint("RssFeed: ${rssFeed.items}");
    } catch (e) {
  ...
  void _getPostsFromNYTimes() async {
    try {
      ...
      String url =
          'https://api.nytimes.com/svc/topstories/v2/home.json?api-key=${TopNewsOverViewScreenSmall.apiKey}';
      http.Response response = await http.get(url);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:
        ...
        itemBuilder: (context, i) => posts.length > 0
            ? NewsItem(
                title: posts[i]['title'],
                url: posts[i]['url'],
                multimedia: posts[i]['multimedia'],
